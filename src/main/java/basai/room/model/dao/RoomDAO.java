...
public class RoomDAO implements RoomInterface{
    ...
    @Override
    public ArrayList<RoomDTO> getAllUserRooms() {
    ...
    @Override
    public ArrayList<RoomDTO> filterRooms(String keyword, String city, String furnishing, String roomType, String[] facilities, String sort) {
        ArrayList<RoomDTO> rooms = new ArrayList<>();
        ...
        if (keyword != null && !keyword.trim().isEmpty()) {
            sql.append(" AND (title LIKE ? OR city LIKE ? OR address LIKE ?)");
        ...
        if (furnishing != null && !furnishing.trim().isEmpty()) {
            sql.append(" AND furnishing_status = ?");
            params.add(furnishing.trim());
        }
        ...
        if (facilities != null && facilities.length > 0) {
            for (String facility : facilities) {
                ...
                params.add("%" + facility.trim() + "%");
            }
        }

        if ("price_desc".equals(sort)) {
            sql.append(" ORDER BY rent_price DESC");
        } else if ("latest".equals(sort)) {
            sql.append(" ORDER BY created_at DESC");
        } else {
            sql.append(" ORDER BY rent_price ASC");
        }
    ...
    @Override
    public ArrayList<RoomDTO> getRoomsByOwner(int ownerId) {
        String sql = "SELECT r.*, u.name AS owner_name FROM rooms r join users u on r.owner_id = u.user_id where r.owner_id=?";
